		...
		foreach($entries as $row){
				    ...
				    ->setRowHeight(20);  			
						$objPHPExcel->setActiveSheetIndex(0)
				            ...
				            ->setCellValue('B'.$line, ucwords(strtolower($row->asistenteName))." ".ucwords(strtolower($row->asistentePrimerApellido))." ".ucwords(strtolower($row->asistenteSegundoApellido)) )
				            ->setCellValue('C'.$line, $row->asistentePrimerApellido." ".$row->asistenteSegundoApellido)
				            ->setCellValue('D'.$line, $row->asistenteCiudad)
				            ->setCellValue('E'.$line, ucwords(strtolower($row->asistenteCentro)))
				            ->setCellValue('F'.$line, ucwords(strtolower($row->asistenteCargo)))
